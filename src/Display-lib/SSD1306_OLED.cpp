...
*/
SSD1306::SSD1306(int16_t oledwidth, int16_t oledheight, std::shared_ptr<I2c::PicoI2C> i2cDevice) :SSD1306_graphics(oledwidth, oledheight)
{
...
void SSD1306::OLEDinit()
 {
        vTaskDelay(pdMS_TO_TICKS(SSD1306_INITDELAY));
...
void SSD1306::I2C_Write_Byte(uint8_t value, uint8_t cmd)
{
	...
	//ReasonCodes = bcm2835_i2c_write(ByteBuffer, 2);
        uint ret =  i2cDevice->write(_I2C_address, &ByteBuffer[0], 2);
        if(ret != 2){
            //Problems
        }
	while(ReasonCodes != 0)
	{//failure to write I2C byte ,Error handling retransmit
		if (_I2C_DebugFlag == true)
		{
			...
			printf("bcm2835I2CReasonCodes :: Error code :: %u\n", +ReasonCodes);
                ...
                vTaskDelay(pdMS_TO_TICKS(_I2C_ErrorDelay)); // delay mS
...
{
    uint8_t addr = _I2C_address;
    uint8_t rxdata;
    uint ret = i2cDevice->read(addr, &rxdata, 1);
    if (ret == 1)
    {
